...
from strategy.indicators import normalize_ohlcv, add_5m_indicators, add_15m_indicators
        ...
        while True:
                ...
                for segment, rows in by_segment.items():
                    ...
                    for pp in rows:
                        ...
                        if mark is None or mark <= 0:
                            logger.warning("Paper position mark unavailable | %s | sec=%s", pp.underlying, pp.security_id)
                            continue
                        ...
                        for ev in events:
                            logger.info(
                                "📘 PAPER %s | %s | %s | qty %d @ %.2f | pnl ₹%.2f | reason=%s | remaining=%d | stop=%.2f",
            ...
            for symbol in config.watchlist:
                try:
                    ...
                    full_signals += 1
                    logger.info("🌟 %s signal %s | score %.0f | model=%s", result.direction, symbol, result.score_pct, result.model)
                    ...
                    logger.info(
                        "🏆 PAPER ENTRY %s | %s | %s health %.0f conf %s | qty %d | entry %.2f | target1 %.2f | stop %.2f | trade_id=%s",
